import { useContext } from "react";
import { useState } from "react";
import "./App.css";

import Expenses from "./components/Expenses/Expenses.component";
import NewExpense from "./components/NewExpenses/NewExpense/NewExpense.component";
import { ExpenseContext } from "./Contexts/Expense.context";
const Dummy_expenses = [
  {
    id: "e1",
    title: "Toilet Paper",
    amount: 94.12,
    date: new Date(2020, 7, 14),
  },
  { id: "e2", title: "New TV", amount: 799.49, date: new Date(2021, 2, 12) },
  {
    id: "e3",
    title: "Car Insurance",
    amount: 294.67,
    date: new Date(2021, 2, 28),
  },
  {
    id: "e4",
    title: "New Desk (Wooden)",
    amount: 450,
    date: new Date(2021, 5, 12),
  },
];
function App() {
  // const [expenses, setExpenses] = useState(Dummy_expenses);
  const { expenseState, setExpenseState } = useContext(ExpenseContext);
  // const { expenseState, setExpenseState } = useContext(ExpenseContext);

  function saveExpenseHandler(expense) {
    setExpenseState([expense, ...expenseState]);
  }
  return (
    <div className="App">
      <NewExpense saveExpense={saveExpenseHandler} />
      <Expenses expenses={expenseState}></Expenses>
    </div>
  );
}

export default App;
